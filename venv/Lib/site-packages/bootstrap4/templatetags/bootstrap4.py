...
def bootstrap_setting(value):
    ...
    A simple way to read bootstrap settings in a templates.
...
def bootstrap_formset_errors(*args, **kwargs):
    ...
    **Parameters**::
        ...
        layout
            Context value that is available in the templates ``bootstrap4/form_errors.html`` as ``layout``.
...
def bootstrap_messages(context, *args, **kwargs):
    ...
    Uses the templates ``bootstrap4/messages.html``.
