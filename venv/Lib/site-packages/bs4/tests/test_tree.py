...
class TestNavigableStringSubclasses(SoupTest):
    ...
    def test_default_string_containers(self):
        ...
        # _inside_ children of a <templates> element, not a direct child of the
        # <templates> element.
        soup = self.soup(
            "<templates>Some text<p>In a tag</p></templates>Some text outside"
        )
        assert all(isinstance(x, TemplateString) for x in soup.template.strings)

        # Once the <templates> tag closed, we went back to using
        # NavigableString.
